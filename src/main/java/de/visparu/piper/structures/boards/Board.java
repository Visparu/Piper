...
public class Board
{
	...
	private Map<Field, Point>	coordinates	= new HashMap<>();
	...
	public Board(int width, int height, int startTimer, float progressIncrement, int[] fixedPieces, int entries,
			int exits, Random rand)
	{
	...
	public void render(Graphics2D g2d)
	{
		...
		for (int y = 0; y < this.fields.length; y++)
		{
			for (int x = 0; x < this.fields[y].length; x++)
			{
				...
				BufferedImage img = new BufferedImage((int) Field.SIZE, (int) Field.SIZE, BufferedImage.TYPE_INT_RGB);
	...
	private void incrementPipeProgress(Field field, Set<Pipe> usedPipes)
	{
		...
		usedPipes.add(pipe);
		float nextIncrement = pipe.increaseProgress(
				Input.isKeyDown(KeyEvent.VK_SPACE) ? Settings.ACCELERATED_PROGRESS_INCREMENT : this.progressIncrement);
	...
	private void spread(Field field, Set<Pipe> usedPipes)
	{
		if (this.exitFields.contains(field))
		{
			return;
		...
		{
			if (pipe.getEntryPoints().contains(d))
			{
				continue;
			}
			...
			switch (d)
			{
				case EAST:
				{
					...
					if (nextPipe == null || !nextPipe.getOpeningDirections().contains(Direction.WEST))
					{
				...
				case NORTH:
				{
					...
					Pipe nextPipe = nextField.getPipe();
					if (nextPipe == null || !nextPipe.getOpeningDirections().contains(Direction.SOUTH))
					{
						this.lose(field, nextField);
						return;
					}
				...
				case SOUTH:
				{
					...
					nextField = this.fields[p.y + 1][p.x];
					Pipe nextPipe = nextField.getPipe();
					if (nextPipe == null || !nextPipe.getOpeningDirections().contains(Direction.NORTH))
					{
						this.lose(field, nextField);
						return;
					}
				...
				case WEST:
				{
					...
					nextField = this.fields[p.y][p.x - 1];
					Pipe nextPipe = nextField.getPipe();
					if (nextPipe == null || !nextPipe.getOpeningDirections().contains(Direction.EAST))
					{
						this.lose(field, nextField);
						return;
					}
		...
		{
			if (exitField.getPipe().getProgress() == 0.0F)
			{
	...
	public boolean hasLeftoverLeak(Field field, Direction inputDirection)
	{
		Pipe pipe = field.getPipe();
		if (pipe == null)
		{
		...
		if (!pipe.getOpeningDirections().contains(inputDirection))
		{
			return true;
		}
		...
		for (Direction d : pipe.getOpeningDirections())
		{
			...
			if (nextPipe.getEntryPoints().contains(nextEntryPoint))
			{
	...
	public void lose(Field from, Field to)
	{
		if (to == null)
		{
